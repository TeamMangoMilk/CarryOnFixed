...
package mangomilk.carryon.client.render;
...
import mangomilk.carryon.client.modeloverride.ModelOverride;
import mangomilk.carryon.client.modeloverride.ModelOverrideHandler;
...
import mangomilk.carryon.common.scripting.CarryOnScript.ScriptRender;
		...
		if(carry.getActiveScript().isPresent())
		{
			...
			if(render.renderNameEntity().isPresent()) {
				...
				if (renderedEntity != null)
					...
					Constants.LOG.warn("Could not create CarryOnRevamped render entity '{}', using carried entity instead", render.renderNameEntity().get().location());
			}
